...
class WorkbenchLocalServerTests(RuntimeTestCase):
    ...
    def test_image_workspace_page_and_api_show_real_source_assets(self) -> None:
        ...
        self.assertIn('id="imageTrialCommand"', page)
        self.assertIn('id="copyImageTrialCommand"', page)
        self.assertIn("单件试图命令 (Single-Product Trial Command)", page)
        self.assertIn("本次只允许领取 1 件", page)
        self.assertIn("不得预领、并发领取或继续领取第二件", page)
        self.assertIn("进入 manual_review_required 后立即停止", page)
