...
10. Inspect pixels against locked evidence and accepted slots. `main_01` has zero copy. Add only allowed verified Russian facts locally to the other approved recipes; each fact cites a locked supplier SHA-256, and a number requires `numeric_verified=true`. Write labels in natural Russian sentence case except for standard abbreviations. The Russian headline must be the clear information focus, not tiny copy stranded inside an oversized panel.
...
- Local panels must size to their verified copy. Reject a layout with an oversized empty rail, a weak Russian hierarchy, or copy that is not comfortably readable in a 360-pixel preview.
