...
<body>
  ...
  <script>
    ...
    function render(data) {{
      ...
      const blockerLabels = {{ supplier_identity:"1688 商品身份恢复", category_template:"类目模板错配", required_attributes:"必填属性", original_content:"原创俄文内容", bootstrap_image:"锁定 1688 原图", pricing:"价格与包装证据" }};
      pageItems.forEach((item) => {{
        ...
        if (item.supplier_recovery_status) {{
          notice.textContent = item.supplier_recovery_status === "supplier_sku_confirmation_required" ? "1688 商品已经重新采集，但还没有锁定真实 SKU；锁定后会自动重建供应商真值、类目和字段模板。" : "这件商品的 1688 供应商身份链已失效，需要重新采集同款商品。";
          const recoverSupplier = document.createElement("a"); recoverSupplier.className = "inline-action"; recoverSupplier.href = `/batches/${{encodeURIComponent(runId)}}/supplier-review`; recoverSupplier.textContent = "返回供应商审核并锁定 SKU"; notice.append(recoverSupplier);
        }} else {{
          notice.textContent = item.template_ready ? `已读取完整 ${{item.attribute_schema_count || 0}} 字段模板：客观事实映射 ${{item.mapped_attribute_count || 0}} 项，待智能生成/规范 ${{item.rewrite_required_count || 0}} 项，缺少必填 ${{item.missing_required_fact_count || 0}} 项，可选缺失 ${{item.missing_optional_fact_count || 0}} 项，可选素材未提供 ${{item.not_applicable_count || 0}} 项。` : `检测到类目模板错配：${{item.category_path || "未知模板"}}。本件停止映射，避免把错误字段写进草稿。`;
          if (!item.template_ready) {{ if (item.category_confirmation_required && item.category_confirmation_url) {{ const confirmCategory = document.createElement("a"); confirmCategory.className = "inline-action"; confirmCategory.href = item.category_confirmation_url; confirmCategory.target = "_blank"; confirmCategory.rel = "noopener"; confirmCategory.textContent = "确认正确类目"; notice.append(confirmCategory); }} else {{ const refresh = document.createElement("button"); refresh.type = "button"; refresh.className = "inline-action"; refresh.textContent = "重新解析正确类目模板"; refresh.addEventListener("click", async () => {{ refresh.disabled = true; refresh.textContent = "正在解析..."; try {{ await api(`/api/batches/${{encodeURIComponent(runId)}}/attribute-template/refresh`, {{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{seed_id:item.seed_id}})}}); window.location.reload(); }} catch (error) {{ refresh.textContent = error.message || "解析失败"; refresh.disabled = false; }} }}); notice.append(refresh); }} }}
        }}
...
function render(items){{const root=document.getElementById("items");root.replaceChildren();const message=document.getElementById("message");if(!items.length){{message.textContent="没有需要人工确认的商品，正在返回工作台...";window.location.replace(`/batches/${{encodeURIComponent(runId)}}/upload`);return;}}message.textContent=`待确认 ${{items.length}} 件`;items.forEach(item=>{{const truth=item.supplier_truth||{{}};const title=truth.subject?.value||truth.supplier_title||item.slot_id;const images=truth.selected_sku_images||[];const card=el("article",undefined,"card");const image=document.createElement("img");image.src=images[0]||truth.primary_image_url||"";image.alt=title||"商品图片";image.referrerPolicy="no-referrer";const body=el("div");body.append(el("h2",title),el("div",`1688 Offer：${{truth.supplier_offer_id||"-"}}　SKU：${{truth.supplier_sku_id||"-"}}`,"meta"));(item.candidates||[]).forEach((option,index)=>{{const label=el("label");const radio=document.createElement("input");radio.type="radio";radio.name=`category-${{item.slot_id}}`;radio.value=JSON.stringify(option);radio.checked=index===0;const categoryLabel=option.display_category_path_zh||(option.matching_language==="ZH_HANS"?(option.matched_category_path||option.category_path||option.category_name):"")||`官方类目候选 ${{index+1}}`;label.append(radio,document.createTextNode(` ${{categoryLabel}}（${{option.description_category_id}} / ${{option.type_id}}）`));body.append(label);}});const button=el("button","确认此类目");button.type="button";button.onclick=()=>confirmItem(item,button);body.append(button);card.append(image,body);root.append(card);}});}}
...
def create_handler(
    ...
    class WorkbenchRequestHandler(BaseHTTPRequestHandler):
        ...
        def _browser_task(self, run_id: str) -> dict[str, Any]:
            ...
            recovery = service.reconcile_supplier_recovery_state(run_id)
            if not recovery.ok:
            ...
            return {
                ...
                "code": "browser_task.none",
                "message": (
                    "The re-collected supplier product is waiting for exact SKU confirmation."
                    if sku_confirmation_seed_ids
                    else "No browser task is pending for this batch."
                ),
                "data": {
                    "run_id": run_id,
                    "created_at": run.get("created_at"),
                    ...
                    "sku_confirmation_seed_ids": sku_confirmation_seed_ids,
                },
                "errors": [],
            }
