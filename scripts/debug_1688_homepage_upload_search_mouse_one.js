...
async function main() {
  ...
  const outDir = path.resolve("runtime", "direct-playwright", "upload-search-mouse-debug");
  ...
  await page.mouse.move(1065, 140);
  await page.mouse.click(1065, 140);
  const chooser = await chooserPromise;
  if (chooser) await chooser.setFiles(imagePath);
  else await page.setInputFiles("#img-search-upload", imagePath);
  await page.waitForTimeout(5000);
  await page.mouse.move(1098, 403);
  await page.mouse.down();
  await page.waitForTimeout(150);
  await page.mouse.up();
  await page.waitForTimeout(25000);
  await page.screenshot({ path: path.join(outDir, "after-search-mouse.png"), fullPage: false });
  ...
  fs.writeFileSync(path.join(outDir, "after-search-mouse.json"), JSON.stringify(result, null, 2), "utf8");
