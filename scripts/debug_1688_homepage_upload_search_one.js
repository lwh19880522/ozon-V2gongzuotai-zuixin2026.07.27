...
async function main() {
  ...
  const outDir = path.resolve("runtime", "direct-playwright", "upload-search-debug");
  ...
  await page.getByText("搜索图片", { exact: true }).click({ timeout: 30000 });
  await page.waitForTimeout(20000);
  await page.screenshot({ path: path.join(outDir, "after-search.png"), fullPage: false });
  ...
  fs.writeFileSync(path.join(outDir, "after-search.json"), JSON.stringify(result, null, 2), "utf8");
