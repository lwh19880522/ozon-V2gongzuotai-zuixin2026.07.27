...
async function main() {
  ...
  const outDir = path.resolve("runtime", "direct-playwright", "upload-debug");
  ...
  const method = chooser ? "filechooser" : "setInputFiles";
  if (chooser) {
  ...
  await page.screenshot({ path: path.join(outDir, "after-upload.png"), fullPage: false });
  const result = await page.evaluate(() => ({
    ...
    fileInputValue: document.querySelector("#img-search-upload")?.value || null,
  }));
  result.method = method;
  fs.writeFileSync(path.join(outDir, "after-upload.json"), JSON.stringify(result, null, 2), "utf8");
